...
try:
    if args.input:
        ...
        for issue in issues_links:
            d = comic.get_pages_links(issue)
        
        #with ThreadPoolExecutor(thread_name_prefix='downloader', max_workers=1) as executor:
        #    results = executor.map(comic.get_pages_links, issues_links)
        
        if args.verbose:
            print(comic.issue_pages)

        if not args.nodownload:
            
            with ThreadPoolExecutor(thread_name_prefix='downloader', max_workers=20) as executor:
                results = executor.map(comic.download_issue, comic.issue_pages, timeout=60)
    
            with ThreadPoolExecutor(thread_name_prefix='downloader', max_workers=20) as executor:
                results = executor.map(makepdf, issue_data, timeout=60)
